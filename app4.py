...
DEFAULT_FILL_RATE = 0.70        # % of ad slots filled
...
def dollars_per_hour(ecpm: float,
    ...
    """
    Convert eCPM (USD per 1,000 impressions) into revenue per viewing hour.
    """
...
def convert_minutes_to_revenue(bucket: str,
    ...
    If bucket has no eCPM (e.g., Live+7), returns NaN.
    """
...
with tab1:
    ...
    with col3:
        # show which model is actually used + its WAPE if we have it
        w_row = None
        if not winners.empty and bucket in set(winners["bucket"]):
