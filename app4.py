...
st.sidebar.dataframe(
    pd.DataFrame(
        [{"Bucket": b, "eCPM ($)": v} for b, v in BUCKET_ECPM.items()]
    ),
    hide_index=True,
...
with tab1:
    ...
    if metric_view == "Revenue ($)" and bucket in BUCKET_ECPM:
        y_plot = convert_minutes_to_revenue(
            bucket,
            y,
            avails_per_hour=avails_per_hour,
            fill_rate=fill_rate
        )
        fc_plot = convert_minutes_to_revenue(
            bucket,
            fc,
            avails_per_hour=avails_per_hour,
            fill_rate=fill_rate
        ...
        if metric_view == "Revenue ($)" and bucket not in BUCKET_ECPM:
            st.info("Revenue view is only configured for VOD/FAST buckets; showing minutes for this bucket.")
        y_plot = y
        fc_plot = fc
    ...
    with c1:
        ...
        ax.plot(y.index, y_plot.values, label="Actual")
        ax.plot(fc.index, fc_plot.values, "--", label=f"Forecast ({selected_model})")
        ax.set_xlabel("Month")
    ...
    # Note: forecast CSV remains in minutes (yhat), which is usually what you want for modeling
